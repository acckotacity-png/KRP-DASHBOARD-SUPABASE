...
const MAIN_HEADERS = ["Timestamp","INVOICE NO.","DATE","CONTACT NO. OR NAME","CUSTOMER NAME","BANK OWNER NAME","STATE","PURPOSE","SERVICE CHARGE REMARKS","LOGIN ID","DEALING AMOUNT","AMOUNT DENO","RECEIVED AMOUNT","ID ACTIVATION AMOUNT","UPLOADING OR SETUP AMOUNT","UTR / TRN NO.","PAYMENT STATUS","REMARKS"];
const MONTH_HEADERS = ["DATE","MONTH","TOTAL ID","WORKING AMT","TRANSFER AMT","MONTHLY AMT","SETUP AMOUNT","REMARKS"];
const NOTE_HEADERS = ["Task Date","Contact No","State","Name","ID","Password","Task Description","Task Status","Payment Date","Dealing Amount","Received Amount","Remaining Amount","Payment Status","Reminder Date"];
...
function mainRow(r) { return [r.created_at,r.invoice_no,r.entry_date,r.contact_name,r.customer_name,r.bank_owner,r.state,r.purpose,r.service_remarks,r.login_id,n(r.dealing_amount),s(r.amount_deno),n(r.received_amount),n(r.id_activation_amount),n(r.uploading_amount),s(r.utr_no),r.payment_status,r.remarks]; }
...
function monthRow(r) { return [r.entry_date,r.month,n(r.total_id),n(r.working_amount),n(r.transfer_amount),n(r.monthly_amount),n(r.setup_amount),r.remarks]; }
function noteRow(r) { const remaining=Math.max(0,n(r.dealing_amount)-n(r.received_amount)); const status=remaining<=0&&n(r.dealing_amount)>0?"PAID":n(r.received_amount)>0?"PARTIAL":"PENDING"; return [r.task_date,r.contact_no,r.state,r.customer_name,r.login_id,r.password_text,r.task_description,r.task_status,r.payment_date,n(r.dealing_amount),n(r.received_amount),remaining,status,r.reminder_date]; }
...
function recalcUdhari(rows) {
  ...
  const output=[];
  for(const r of sorted){const event=isoDate(r.entry_date)||today,key=mobile(r.mobile_no)||s(r.customer_name).trim().toLowerCase();const a=accounts[key]||{principal:0,interest:0,rate:0,type:"MONTHLY",last:event,due:null};accrue(a,event);const payment=s(r.transaction_type).toUpperCase()==="PAYMENT MILA";if(payment){let pay=n(r.payment_amount),ip=Math.min(a.interest,pay);a.interest-=ip;pay-=ip;a.principal=Math.max(0,a.principal-pay);}else{a.principal+=n(r.udhar_amount);a.rate=n(r.interest_rate);a.type=s(r.interest_type||"MONTHLY").toUpperCase();const start=isoDate(r.interest_start_date)||event;a.last=start>event?start:event;}const due=isoDate(r.due_date);if(due)a.due=due;const item={r,a,payment};output.push(item);a.latest=item;accounts[key]=a;}
  Object.values(accounts).forEach(a=>accrue(a,today,true));
  return output.map(({r,a,payment})=>{const total=round(a.principal+a.interest);const status=total<=0?"PAID":a.due&&a.due<today?"OVERDUE":n(r.payment_amount)>0?"PARTIAL":"PENDING";return [r.id,r.entry_date,r.customer_name,r.mobile_no,r.address,r.transaction_type,r.description,n(r.udhar_amount),n(r.payment_amount),round(a.principal),r.due_date,r.payment_mode,r.reference_no,status,r.reminder_date,r.remarks,r.created_by_email,r.created_at,payment?0:n(r.interest_rate),payment?"":r.interest_type,payment?"":r.interest_start_date,dateText(today),round(a.interest),total];});
}
function expenseData(expenses,budgets){const names=["JANUARY","FEBRUARY","MARCH","APRIL","MAY","JUNE","JULY","AUGUST","SEPTEMBER","OCTOBER","NOVEMBER","DECEMBER"];const key=v=>{const m=s(v).match(/^\d{2}-(\d{2})-(\d{4})$/);return m?`${names[+m[1]-1]}-${m[2]}`:""};return expenses.map(r=>{const monthName=key(r.entry_date);const b=budgets.find(x=>s(x.month).toUpperCase()===monthName&&x.category===r.category&&x.active);const budget=n(b?.monthly_budget);const related=expenses.filter(x=>key(x.entry_date)===monthName&&x.category===r.category);const spent=related.reduce((a,x)=>a+n(x.amount),0);const monthTotal=expenses.filter(x=>key(x.entry_date)===monthName).reduce((a,x)=>a+n(x.amount),0);const used=budget?round(spent*100/budget):0,limit=n(b?.warning_limit)||80;const alert=!budget?"NO BUDGET":used>=100?"OVER BUDGET":used>=limit?"WARNING":"SAFE";const remark=alert==="OVER BUDGET"?`${r.category} expense control karein`:alert==="WARNING"?`${r.category} budget limit ke paas hai`:"";return [r.id,r.entry_date,monthName,r.category,r.sub_category,r.description,r.payment_mode,r.paid_to,n(r.amount),r.reference_no,r.bill_link,r.expense_type,r.priority,budget,round(spent),round(budget-spent),round(monthTotal),used,alert,remark,r.added_by_email,r.created_at];});}
...
async function route(client,user,p){
  ...
  if(action==="getPurposeSettings"){
    const {data,error}=await client.from("business_settings").select("purpose_options,financial_year_options,state_options").eq("id",1).maybeSingle();
    if(error)throw error;
    return {success:true,purposes:Array.isArray(data?.purpose_options)?data.purpose_options:[],years:Array.isArray(data?.financial_year_options)?data.financial_year_options:[],states:Array.isArray(data?.state_options)?data.state_options:[]};
  }
  if(action==="savePurposeSettings"){
    const purposes=parseJson(p.purposes),years=parseJson(p.years),states=parseJson(p.states);
    if(!Array.isArray(purposes)||!Array.isArray(years)||!Array.isArray(states))throw Error("Invalid purpose settings");
    const {error}=await client.from("business_settings").upsert({id:1,purpose_options:purposes,financial_year_options:years,state_options:states,updated_by:user.uid},{onConflict:"id"});
    if(error)throw error;
    return {success:true,purposes,years,states};
  }
