...
const MAIN_HEADERS = ["Timestamp","INVOICE NO.","DATE","CONTACT NO. OR NAME","CUSTOMER NAME","BANK OWNER NAME","STATE","PURPOSE","SERVICE CHARGE REMARKS","LOGIN ID","DEALING AMOUNT","AMOUNT DENO","RECEIVED AMOUNT","ID ACTIVATION AMOUNT","UPLOADING OR SETUP AMOUNT","UTR / TRN NO.","PAYMENT STATUS","REMARKS","CREATED BY"];
...
function recalcUdhari(rows) {
  ...
  return output.map(({r,a,payment})=>{const total=round(a.principal+a.interest);const status=total<=0?"PAID":a.due&&a.due<today?"OVERDUE":n(r.payment_amount)>0?"PARTIAL":"PENDING";return [r.id,r.entry_date,r.customer_name,r.mobile_no,r.address,r.transaction_type,r.description,n(r.udhar_amount),n(r.payment_amount),round(a.principal),r.due_date,r.payment_mode,r.reference_no,status,r.reminder_date,r.remarks,r.created_by_name||r.created_by_email,stampText(r.created_at),payment?0:n(r.interest_rate),payment?"":r.interest_type,payment?"":r.interest_start_date,dateText(today),round(a.interest),total];});
}
